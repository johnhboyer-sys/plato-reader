...
Source: `data/tlg0059/tlg<NNN>/tlg0059.tlg<NNN>.perseus-grc2.xml`. Six files, the works on paragraph flow: tlg002 (Apology), tlg004 (Phaedo), tlg016 (Amatores/Lovers), tlg018 (Charmides), tlg030 (Republic), tlg036 (Letters). Dialogues need no donor — the TLG gives them real `<label type="speaker">` turns. All six resolved as `perseus-grc2`; no `grc1` fallback (grc1 404s for tlg030).

Phaedo (added 2026-09-05) is the mixed case: the TLG labels only the Echecrates/Phaedo frame (34 turns), and Burnet paragraphs the narration between one embedded speech at a time — 592 `<milestone ed="P" unit="para"/>` marks. The donor also wraps every page of the narration in a fresh `<said who="#Φαίδων" rend="merge"><label>ΦΑΙΔ.</label>` (93 labels for 34 real turns); those labels are markup, not text, and `stage1_greek_paras.parse_marks` strips them from a mark's probe (a two-word reply at a page end otherwise carried "ΦΑΙΔ." as its second word and never located). The frame turns are not marks: stage7 pins them as labelled rows inside the mark spine (`turns.build_para_flow`, manifest `greek.paragraphs.spine`).
...
Located marks (stage1 reports `greek paragraphs: X/Y located`): Republic 4234/4234, Apology 73/73, Lovers 140/140, Charmides 358/359, Letters 53/57 — 4858 of 4863 (99.9%). Phaedo: 592 marks; all 592 locate on a stand-in spine cut from the donor's own text (the real TLG count is whatever the next stage1 run prints — record it here). The 5 unlocated are dropped, not guessed: the flow keeps its proportional estimate for those rows (a wrong anchor would cut a row mid-clause; see `turns.build_para_flow`).
