...
Located marks (stage1 reports `greek paragraphs: X/Y located`): Republic 4234/4234, Apology 73/73, Lovers 140/140, Charmides 358/359, Letters 53/57 — 4858 of 4863 (99.9%). Spine-mode works, donor mark counts: Phaedo 592, Symposium 236, Timaeus 53, Critias 12, Menexenus 10, Epinomis 15, Clitophon 2. All locate on a stand-in spine cut from the donor's own text (the real TLG counts are whatever the next stage1 runs print — record them here). Not converted, on purpose: Lysis, Parmenides, Protagoras and Euthydemus, whose narration the OCT already labels with dash turns the dialogue flow pairs (the dash-heavy tail is its own problem), and the Jowett-overlay dialogues (Phaedrus, Gorgias, Crito), whose few marks sit inside speeches and whose compare column is aligned turn-by-turn against the dialogue flow. The 5 unlocated are dropped, not guessed: the flow keeps its proportional estimate for those rows (a wrong anchor would cut a row mid-clause; see `turns.build_para_flow`).
