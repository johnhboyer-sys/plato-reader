...
Source: `data/tlg0059/tlg<NNN>/tlg0059.tlg<NNN>.perseus-grc2.xml`. Twelve files: the works on paragraph flow — tlg002 (Apology), tlg016 (Amatores/Lovers), tlg018 (Charmides), tlg030 (Republic), tlg036 (Letters) — and the works in SPINE mode with labelled turns pinned inside Burnet's marks — tlg004 (Phaedo), tlg011 (Symposium), tlg028 (Menexenus), tlg029 (Clitophon), tlg031 (Timaeus), tlg032 (Critias), tlg035 (Epinomis). Dialogues need no donor — the TLG gives them real `<label type="speaker">` turns. All resolved as `perseus-grc2`; no `grc1` fallback (grc1 404s for tlg030).

Phaedo (added 2026-09-05, with Symposium, Timaeus, Critias, Menexenus, Epinomis and Clitophon the same day) is the mixed case: the TLG labels only the Echecrates/Phaedo frame (34 turns), and Burnet paragraphs the narration between one embedded speech at a time — 592 `<milestone ed="P" unit="para"/>` marks. The donor also wraps every page of the narration in a fresh `<said who="#Φαίδων" rend="merge"><label>ΦΑΙΔ.</label>` (93 labels for 34 real turns); those labels are markup, not text, and `stage1_greek_paras.parse_marks` strips them from a mark's probe (a two-word reply at a page end otherwise carried "ΦΑΙΔ." as its second word and never located). The frame turns are not marks: stage7 pins them as labelled rows inside the mark spine (`turns.build_para_flow`, manifest `greek.paragraphs.spine`).
...
Located marks (stage1 reports `greek paragraphs: X/Y located`): Republic 4234/4234, Apology 73/73, Lovers 140/140, Charmides 358/359, Letters 53/57 — 4858 of 4863 (99.9%). Spine-mode works, on the real TLG spine (laptop build 2026-09-08; stage1 located / stage7 `para_spine` matched): Phaedo 592/592 located, 591/592 matched (the miss is 93a "Οὐδαμῶς.", merged into the row before); Symposium 236/236, 236/236; Timaeus 53/53, 53/53; Critias 12/12, 12/12; Menexenus 10/10, 10/10; Epinomis 15/15, 15/15; Clitophon 2/2, 2/2. Every donor mark located — the donors and the TLG spine have not drifted. Not converted, on purpose: Lysis, Parmenides, Protagoras and Euthydemus, whose narration the OCT already labels with dash turns the dialogue flow pairs (the dash-heavy tail is its own problem), and the Jowett-overlay dialogues (Phaedrus, Gorgias, Crito), whose few marks sit inside speeches and whose compare column is aligned turn-by-turn against the dialogue flow. The 5 unlocated are dropped, not guessed: the flow keeps its proportional estimate for those rows (a wrong anchor would cut a row mid-clause; see `turns.build_para_flow`).
